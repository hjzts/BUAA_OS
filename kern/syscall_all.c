...
int sys_mem_alloc(u_int envid, u_int va, u_int perm)
{
    ...
    if (is_illegal_va(va))
        return -E_INVAL;
    /* Step 2: Convert转变 the envid to its corresponding相应的 'struct Env *' using 'envid2env'. */
...
int sys_ipc_try_send(u_int envid, u_int value, u_int srcva, u_int perm)
{
    ...
    if (srcva != 0) {
        ...
        // TODO:try(sys_mem_map(curenv->env_id, srcva, e->env_id, e->env_ipc_dstva, perm));
...
void do_syscall(struct Trapframe* tf)
{
    ...
    // func是一个函数指针，通过syscall_table找到对应哪一个函数
    // 每个函数都会接受几个参数，最多五个
    // 返回一个int
    int sysno = tf->regs[4];
    ...
    /* Step 5: Invoke调用 'func' with retrieved 检索到的 arguments and store its return value to $v0 in 'tf'.
     */
