...
// 存放进程控制块的物理内存，在操作系统启动的时候就已经准备好了。
...
// 使用了TAILQ结构，双端队列，支持头部插入和取出、尾部插入和取出
...
void env_init(void)
{
    ...
    /* Step 2: Traverse 遍历 the elements of 'envs' array, set their status to 'ENV_FREE' and insert
...
static void load_icode(struct Env* e, const void* binary, size_t size)
{
     ...
     */
    size_t ph_off;
    // 从ehdr->e_phoff 开始遍历 e->phnum个大小为e_phentsize的段头
    ...
    e->env_tf.cp0_epc = (unsigned long)ehdr->e_entry;
}
